...
function Mood() {
  ...
  const QUESTIONS = [
    'How much would you rate your mood today on a scale of 10?',
    'How many hours did you sleep approximately?',
    'On a scale of 10, how stressful was your day?'
  ];
    ...
    <div>
      <div>
        {QUESTIONS.map((question, index) => (
          <div key={index} className=" rounded-full mx-20 my-28 bg-slate-300 px-9 py-7 shadow-xl">
            ...
            <div>
              <select
                value={answers.job}
                onChange={(e) => onOptionChange(["moodss", "sleep", "stress"][index], e.target.value)}
                className=" bg-gray-700 border-gray-400 border-4 rounded-[40px] w-full text-white py-2 px-10 mb-5"
              >
